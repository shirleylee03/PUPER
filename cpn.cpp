...
Bucket operate(Bucket &lbk,Bucket &rbk,string Operator){
    if(lbk.tid != rbk.tid) {
        //强制类型转化
        if(lbk.tid==Integer && rbk.tid==Real){
            lbk.tid=Real;
            lbk.real=lbk.integer;
        }
        else if(lbk.tid==Real && rbk.tid==Integer){
            rbk.tid=Real;
            rbk.real=lbk.integer;
        }
        else {
            throw "ERROR in Bucket booloperate!";
        }
...
//                thread_idexp = thread_v;
        ...
        else if(p->child->place == "pthread_mutex_lock")
        {
            ...
            auto t_row=findT_byid(enter_T[0])->getRow();
...
CPlace* CPN::cal_t2thread_call(string thread,string callT,CPlace *place ,type_callStack call_stack) {
    ...
    //debug+
    if(call_stack.front().front() == "T37" &&call_stack.back().front() == "T47")
        cout<<endl;
    //debug-
...
bool CPN::within_criticalSec(type_T2Call tCall1,type_T2Call tCall2) {
    ...
    auto row1=t1->getRow()!=65535?t1->getRow():map_NoneRow.find(t1->getid())->second;
    auto row2=t2->getRow()!=65535?t2->getRow():map_NoneRow.find(t2->getid())->second;
    ...
    for(auto i=tCall1.second.begin()+1;i!=tCall1.second.end();i++){//begin是线程创建变迁，跳过
        ...
        auto row=t->getRow()!=65535?t->getRow():map_NoneRow.find(t->getid())->second;
...
void CPN::conflictT_add(type_multimapCall2T &mapCall2T, type_T_Call2thread t_call2thread) {
    ...
    for(;iter1!=mapCall2T.end();iter1++){
        if(iter1->first == t_call2thread.first.second) {//Call相同
            ...
            auto row1= findT_byindex(idx1)->getRow();
            auto row2= findT_byindex(idx2)->getRow();
            if(row1==row2){
